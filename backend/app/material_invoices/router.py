...
@router.post("/{project_id}/invoices/{invoice_id}/extract")
def extract_invoice(project_id: str, invoice_id: str) -> dict:
...
def link_invoice(project_id: str, invoice_id: str, payload: LinkInvoiceRequest) -> dict:
    ...
    wall.layers.append(
        MaterialLayer(
            source="invoice",
            material_ref=extracted.get("material"),
            thickness_cm=extracted.get("thickness_cm"),
            r_value=extracted.get("r_value"),
            product_reference=extracted.get("product_reference"),
    ...
    store.save(state)
    return {"wall_id": wall.id, "layers": [layer.model_dump() for layer in wall.layers]}
